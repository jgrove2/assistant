...
def test_identify_faces_returns_known_name() -> None:
    ...
    with patch("src.face_recognizer.face_recognition.face_locations", return_value=[(0, 100, 100, 0)]):
        with patch("src.face_recognizer.face_recognition.face_encodings", return_value=[face_enc]):
            with patch("src.face_recognizer.face_recognition.compare_faces", return_value=[True]):
                with patch("src.face_recognizer.face_recognition.face_distance", return_value=np.array([0.0])):
...
def test_identify_faces_returns_none_for_unknown() -> None:
    ...
    with patch("src.face_recognizer.face_recognition.face_locations", return_value=[(0, 100, 100, 0)]):
        with patch("src.face_recognizer.face_recognition.face_encodings", return_value=[face_enc]):
            with patch("src.face_recognizer.face_recognition.compare_faces", return_value=[False]):
                with patch("src.face_recognizer.face_recognition.face_distance", return_value=np.array([0.9])):
                    result = recognizer.identify_faces(dummy_frame)
                    assert result == [None]
...
def test_identify_faces_returns_empty_for_no_faces() -> None:
    ...
    with patch("src.face_recognizer.face_recognition.face_locations", return_value=[]):
        result = recognizer.identify_faces(dummy_frame)
        assert result == []
...
def test_register_face_saves_and_updates(tmp_path: Path) -> None:
    ...
    with patch("src.face_recognizer.FACES_DIR", tmp_path):
        with patch("src.face_recognizer.KNOWN_FACES_FILE", tmp_path / "known_faces.json"):
            with patch("src.face_recognizer.FACES_IMAGES_DIR", tmp_path / "images"):
                with patch("src.face_recognizer.face_recognition.face_encodings", return_value=[encoding]):
                    with patch("src.face_recognizer.cv2.imwrite"):
...
def test_register_face_returns_false_on_no_face() -> None:
    ...
    with patch("src.face_recognizer.face_recognition.face_encodings", return_value=[]):
        result = recognizer.register_face("Justin", dummy_frame)
...
def test_encoding_roundtrip() -> None:
    ...
    try:
        ...
        with open(tmp_faces_file, "w") as f:
            json.dump(data, f)
        with patch("src.face_recognizer.KNOWN_FACES_FILE", tmp_faces_file), \
             patch("src.face_recognizer.FACES_DIR", tmp_path):
            fr = FaceRecognizer()
...
def test_register_face_rejects_path_traversal() -> None:
    ...
    try:
        tmp_path = Path(tmp)
        with patch("src.face_recognizer.FACES_DIR", tmp_path), \
             patch("src.face_recognizer.FACES_IMAGES_DIR", tmp_path / "images"), \
             patch("src.face_recognizer.KNOWN_FACES_FILE", tmp_path / "known_faces.json"):
