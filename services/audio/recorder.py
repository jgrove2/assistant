...
def test_listen_timeout_raises() -> None:
    ...
    with patch("recorder.pyaudio.PyAudio", return_value=mock_audio), \
         patch("recorder.time.monotonic", side_effect=[0.0, 1.0, 1.0, 1.0]):
        try:
            recorder.record()
...
def test_default_timeout_raises_listen_timeout() -> None:
    ...
    with patch("recorder.pyaudio.PyAudio", return_value=mock_audio), \
         patch("recorder.time.monotonic", side_effect=monotonic_values):
        try:
            recorder.record()
            assert False, "Expected ListenTimeout"
        except ListenTimeout:
...
def test_vad_detects_speech_and_stops_on_silence() -> None:
    ...
    with patch("recorder.pyaudio.PyAudio", return_value=mock_audio), \
         patch("recorder.time.monotonic", side_effect=times):
